...
class MetadataVisitor(asdl.VisitorBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.metadata.sums.add("int")
        self.metadata.sums.add("identifier")
...
class FunctionVisitor2(PrototypeVisitor):
    """Visitor to generate constructor functions for AST."""
    ...
    def emit_function(self, name, ctype, args, attrs, union=True):
        ...
        argstr = ", ".join(["%s %s" % (atype, aname)
                            for atype, aname, opt in args + attrs])
        if argstr:
            argstr = "struct ast_state* state, " + argstr
        else:
            argstr = "struct ast_state* state"
        self.emit("%s" % ctype, 0)
        emit("%s(%s)" % (ast_func_name(name) + "Heap", argstr))
        emit("{")
        emit("%s p;" % ctype, 1)
        for argtype, argname, opt in args:
            if not opt and argtype != "int":
        ...
        emit(f"p = ({ctype})_PyObject_New((PyTypeObject *)state->_{name}_type);", 1)
        #emit("p = (%s)_PyArena_Malloc(arena, sizeof(*p));" % ctype, 1);
        emit("if (!p)", 1)
        emit("return NULL;", 2)
        if union:
...
class Obj2ModPrototypeVisitor(PickleVisitor):
    def visitModule(self, mod):
        self.emit_seq_proto("int", simple=True)
        self.emit_seq_proto("identifier")
        super().visitModule(mod)

    def emit_seq_proto(self, name, simple=False):
        if simple:
            t = "asdl_int_seq"
        else:
            t = f"asdl_{name}_seq"
    ...
    def visitProduct(self, prod, name):
        ...
        code = "static int obj2imm_%s(struct ast_state *state, PyObject* obj, %s* out);"
        self.emit(code % (name, get_c_type(name)), 0)

    def visitSum(self, sum, name):
        self.emit_seq_proto(name, is_simple(sum))
...
class Obj2ModVisitor(PickleVisitor):
    ...
    def visitModule(self, mod):
        self.emit_seq("int", simple=True)
        self.emit_seq("identifier")
        super().visitModule(mod)
    ...
    def funcHeaderImm(self, name):
        ctype = get_c_type(name)
        self.emit("int", 0)
        self.emit("obj2imm_%s(struct ast_state *state, PyObject* obj, %s* out)" % (name, ctype), 0)
        self.emit("{", 0)
        self.emit("int isinstance;", 1)
        self.emit("", 0)

    def emit_seq(self, name, simple=False):
        ctype = get_c_type(name)
        if simple:
            t = "asdl_int_seq"
        else:
    ...
    def buildArgsHeap(self, fields):
        return ", ".join(["state"] + fields)
    ...
    def visitSum(self, sum, name):
        self.emit_seq(name, is_simple(sum))
        if is_simple(sum):
    ...
    def visitProduct(self, prod, name):
        ...
        ctype = get_c_type(name)
        self.emit("int", 0)
        self.emit(f"obj2imm_%s(struct ast_state *state, PyObject* obj, {ctype}* out)" % (name, ), 0)
        self.emit("{", 0)
        ...
        self.emit("PyObject* tmp = NULL;", 1)
        for f in prod.fields:
            self.visitField2(f, name, prod=prod, depth=1)
        for a in prod.attributes:
            self.visitField2(a, name, prod=prod, depth=1)
        args = [f.name for f in prod.fields]
        args.extend([a.name for a in prod.attributes])
    ...
    def visitField2(self, field, name, sum=None, prod=None, depth=0):
        ctype = get_c_type(field.type)
        line = "if (PyObject_GetOptionalAttr(obj, state->%s, &tmp) < 0) {"
        self.emit(line % field.name, depth)
        self.emit("return -1;", depth+1)
...
class AnnotationsVisitor(PickleVisitor):
    ...
    def emit_annotations(self, name, fields):
        ...
        self.emit_annotations_error(name, 1)
        self.emit(f'cond = PyObject_SetAttrString(state->_{name}_type, "_field_types", {name}_annotations) == 0;', 1)
        self.emit_annotations_error(name, 1)
        self.emit(f'cond = PyObject_SetAttrString(state->_{name}_type, "__annotations__", {name}_annotations) == 0;', 1)
        self.emit_annotations_error(name, 1)
...
""", 0, reflow=False)
        ...
        for dfn in mod.dfns:
            self.visit(dfn)
        self.create_type("int_seq", "int_seq")
        self.create_type("identifier_seq", "identifier_seq")
        self.file.write(textwrap.dedent('''
    ...
    def visitConstructor(self, cons, name, simple):
        ...
        if simple:
            ...
            self.emit("state->_%s_singleton = PyType_GenericNew((PyTypeObject *)"
                      "state->_%s_type, NULL, NULL);" %
                             (cons.name, cons.name), 1)
            self.emit("if (!state->_%s_singleton) return -1;" % cons.name, 1)
...
class ASTModuleVisitor(PickleVisitor):
    ...
    def visitModule(self, mod):
        ...
        self.emit("return -1;", 2)
        self.emit('}', 1)
        self.emit('if (PyModule_AddIntMacro(m, PyCF_IMMUTABLE_AST) < 0) {', 1)
        self.emit("return -1;", 2)
        self.emit('}', 1)
...
class NodesDeclVisitor(EmitVisitor):
    def visitModule(self, mod):
        for dfn in mod.dfns:
            self.visit(dfn)

        self.copy_seq_func_proto("int")
        self.copy_seq_func_proto("identifier")

    def visitType(self, type):
        self.visit(type.value, type.name)

    def copy_func_proto(self, type_name, name):
        self.emit(f"{name}_ty _PyAst_{type_name}_Copy({name}_ty self);", 0)
        self.emit(f"PyAPI_FUNC({name}_ty) _PyAst_{name}_Copy({name}_ty node);", 0)

    def copy_seq_func_proto(self, name):
    ...
    def visitSum(self, sum, name, depth = 0):
        ...
        self.copy_func_proto(name, name)
        self.copy_seq_func_proto(name)

        for type in sum.types:
            self.copy_func_proto(type.name, name)
...
class NodesVisitor(EmitVisitor):
    def visitModule(self, mod):
        ...
        self.emit(f"static void AST_dealloc(PyObject *self) {{", 0)
    ...
    def visitSum(self, sum, name, depth = 0):
        ...
        self.emit("switch (node->kind) {", 1)
        for type in sum.types:
            self.emit(f"case {type.name}_kind:", 2)
            self.emit(f"return _PyAst_{type.name}_Copy(node);", 3)
    ...
    def visitProduct(self, product, name, depth = 0):
        self.seq_type(name, f"struct _{name}")

        self.emit_new("->", name, name, product.fields)
...
""", 0, reflow=False)
    ...
    def emit_dealloc_header(self, node_name, type_name, need_obj=True):
        self.emit(f"static void {node_name}_dealloc(PyObject *self) {{", 0)
        if need_obj:
            self.emit(f"{type_name}_ty obj = ({type_name}_ty)self;", 1)

    def emit_dealloc_footer(self):
        self.emit("PyTypeObject *type = Py_TYPE(self);", 1)
        self.emit("type->tp_free(self);", 1)
        self.emit("Py_DECREF(type);", 1)
        self.emit("}", 0)
        self.emit("", 0)

    def emit_type_dealloc(self, container, type, fields, depth):
        for field in fields:
            if field.type in self.metadata.simple_sums or field.type == "int":
                continue
            self.emit(f"Py_XDECREF({container}{field.name});", depth)

    def emit_copy(self, src, dst, seq, type, depth):
        if not seq and (type in self.metadata.simple_sums or type == "int"):
            self.emit(f"{dst} = {src};", depth)
            return
        self.emit(f"if ({src} != NULL) {{", depth)
        if seq:
            assert type != "cmpop"
            self.emit(f"{dst} = _PyAst_{type}_seq_Copy({src});", depth + 1)
        elif type == "identifier" or type == "constant" or type == "string":
            self.emit(f"{dst} = Py_NewRef({src});", depth + 1)
        else:
            self.emit(f"{dst} = _PyAst_{type}_Copy({src});", depth + 1)
        self.emit(f"ERROR_IF_NULL({dst});", depth + 1)
        self.emit("} else {", depth)
    ...
    def emit_seq_copy(self, name):
        ...
        self.emit("goto error;", 2)
        self.emit("}", 1)
        self.emit("res->size = seq->size;", 1)
        self.emit("res->elements = (void**)&res->typed_elements[0];", 1)
        self.emit("for (Py_ssize_t i = 0; i < seq->size; i++) {", 1)
        self.emit_copy("seq->typed_elements[i]", "res->typed_elements[i]", False, name, 2)
        self.emit("}", 1)
        self.emit("return res;", 1)
        self.emit("error:", 1)
        self.emit("Py_XDECREF(res);", 1)
    ...
    def emit_seq_dealloc_header(self, name):
        self.emit(f"static void {name}_seq_dealloc(PyObject *self) {{", 0)
        if name not in self.metadata.simple_sums and name != "int":
            self.emit(f"asdl_{name}_seq *obj = (asdl_{name}_seq *)self;", 1)

    def emit_seq_dealloc(self, name, dealloc_elems=True):
        self.emit_seq_dealloc_header(name)
        if dealloc_elems:
            self.emit("for (Py_ssize_t i = 0; i < obj->size; i++) {", 1)
            self.emit("PyObject *value = (PyObject *)obj->typed_elements[i];", 2)
            self.emit("if (value) {", 2)
            self.emit("Py_DECREF(value);", 3)
            self.emit("}", 2)
            self.emit("}", 1)
        self.emit_dealloc_footer()
        self.emit("", 0)
...
static PyType_Spec _{name}_seq_type_spec = {{
    "ast._{name}_seq",
