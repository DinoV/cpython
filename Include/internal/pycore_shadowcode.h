...
#define INITIAL_POLYMORPHIC_CACHE_ARRAY_SIZE 2
#define POLYMORPHIC_CACHE_SIZE 2
...
#define INLINE_CACHE_CREATED(size)                                            \
    do {                                                                      \
        inline_cache_count++;                                                 \
...
#define LOAD_ATTR_CACHE_MISS_WITH_NAME(opcode, target, name)                  \
    INLINE_CACHE_RECORD_STAT(opcode, misses);                                 \
    res = _PyShadow_LoadAttrInvalidate(                                       \
        shadow, next_instr, owner, name, target);                             \
    if (res == NULL)                                                          \
        return NULL;

#define LOAD_ATTR_CACHE_MISS(opcode, target)                                  \
    LOAD_ATTR_CACHE_MISS_WITH_NAME(opcode, target, entry->name)
...
{
    INLINE_CACHE_RECORD_STAT(LOAD_ATTR_SLOT, hits);
                                    ...
                                    PyObject *owner)
{
    ...
    INLINE_CACHE_TYPE_STAT(entry->type, "split_dict_descr");
    res =
                         ...
                         PyObject *owner)
{
    ...
    if (entry->module == owner) {
        if (entry->version != PYCACHE_MODULE_VERSION(owner)) {
            PyObject *name = _PyShadow_GetOriginalName(shadow, next_instr);
            entry->value = PyDict_GetItemWithError(
                ((PyModuleObject *)owner)->md_dict, name);
            if (entry->value == NULL) {
                LOAD_ATTR_CACHE_MISS_WITH_NAME(LOAD_ATTR_MODULE, NULL, name)
                return res;
            }
        ...
        LOAD_ATTR_CACHE_MISS_WITH_NAME(LOAD_ATTR_MODULE,
            NULL, _PyShadow_GetOriginalName(shadow, next_instr))
    }
    return res;
}
...
#define LOAD_METHOD_CACHE_MISS_WITH_NAME(opcode, target, name)                \
    INLINE_CACHE_RECORD_STAT(opcode, misses);                                 \
    return _PyShadow_LoadMethodInvalidate(                                    \
        shadow, next_instr, obj, name, (PyObject *)target, meth);


#define LOAD_METHOD_CACHE_MISS(opcode, target)                                \
    LOAD_METHOD_CACHE_MISS_WITH_NAME(opcode, target, entry->name)
                         ...
                         PyObject **meth)
{
    ...
    if ((PyObject *)entry->type == obj) {
        /* Cache hit */
        INLINE_CACHE_TYPE_STAT(entry->type, "loadmethod_nodict_type_descr");
                           ...
                           PyObject **meth)
{
    ...
    if (entry->module == obj) {
        if (entry->version != PYCACHE_MODULE_VERSION(obj)) {
            PyObject *name = _PyShadow_GetOriginalName(shadow, next_instr);
            entry->value = PyDict_GetItemWithError(
                ((PyModuleObject *)obj)->md_dict, name);
            if (entry->value == NULL) {
                LOAD_METHOD_CACHE_MISS_WITH_NAME(LOAD_METHOD_MODULE, NULL, name)
            }
    ...
    LOAD_METHOD_CACHE_MISS_WITH_NAME(LOAD_METHOD_MODULE, NULL, _PyShadow_GetOriginalName(shadow, next_instr))
}
